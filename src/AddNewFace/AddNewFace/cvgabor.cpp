...
long CvGabor::mask_width()
{
   ...
   // double dWidth = round(dModSigma*6 + 1);
	  double dWidth = (double)((int)(dModSigma*5 + 1 + 0.5));
...
void CvGabor::conv_img_a(IplImage *src, IplImage *dst, int Type)
{
	...
	CvMat *kernel = cvCreateMat( Width, Width, CV_32FC1 );
	CvMat *rmat = cvCreateMat(width + kernel->width - 1, height + kernel->height - 1, CV_32FC1);
	CvMat *imat = cvCreateMat(width + kernel->width - 1, height + kernel->height - 1, CV_32FC1);
	IplImage *tmpImage = cvCreateImage(cvSize(imat->rows,imat->cols), IPL_DEPTH_8U, 1 ); 
	...
	case CV_GABOR_MAG:
		...
		for (int i = 0; i < rmat->width; i++)
		{
			for (int j = 0; j < rmat->height; j++)
			{
				re = cvGetReal2D((CvMat*)rmat, i, j);
				im = cvGetReal2D((CvMat*)imat, i, j);
				ve = sqrt(re*re + im*im);
				/*cvSetReal2D( (CvMat*)mat, i, j, ve );*/
				cvSetReal2D( (CvMat*)tmpImage, i, j, ve );
			}
		}
		cvNamedWindow("juanji");
		cvShowImage("juanji",tmpImage);
		cvWaitKey(0);
		break;
	case CV_GABOR_PHASE:
		break;
